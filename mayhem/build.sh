...
# DEBUG_FLAGS: emit DWARF ≤ 3 symbols (Mayhem's triage can't read DWARF ≥ 4; clang-19 defaults to
# DWARF-5 with plain -g). Threaded AFTER $SANITIZER_FLAGS so -gdwarf-3 wins over any -g in SANITIZER_FLAGS.
...
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $HARNESS_CFLAGS \
...
# ── 3) Behavioral oracle binary (normal flags, no sanitizers) ─────────────────
# A small standalone C program that exercises M*LIB containers (array/string/list)
# with KNOWN-ANSWER operations and prints the computed values to stdout.  test.sh
# greps for exact strings; a no-op / exit(0) patch produces wrong or empty output
# and is caught — this is the anti-reward-hack behavioral oracle (§6.3).
echo "build.sh: building behavioral oracle (mayhem/oracle_behavior)"
$CC -std=c99 -D_GNU_SOURCE -O -I"$SRC" \
    "$SRC/mayhem/oracle_behavior.c" \
    -o /mayhem/oracle_behavior

echo "build.sh: built /mayhem/fuzz_string (+ -standalone), M*LIB test suite, and oracle_behavior"
ls -l /mayhem/fuzz_string /mayhem/fuzz_string-standalone /mayhem/oracle_behavior
